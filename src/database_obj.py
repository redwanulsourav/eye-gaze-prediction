...
class Database:
    ...
    def findmin(self, eval_mode, key):
        if eval_mode == '':
            if key in self.db_dict:
                ...
                for r in runs:
                    f = open(f'{self.prefix}/{r}/epochs/49/history.json')
                    contents = f.read()
                    f.close()

                    data_dict = json.loads(contents)
                    min_error =data_dict['average_loss']
    ...
    def findmin_online(self, eval_mode, key):
        if eval_mode == '':
            if key in self.db_dict:
                runs = self.db_dict[key]
                runs = [int(x) for x in runs]
    ...
    def findargmin(self, eval_mode, key):
        if eval_mode == '':
            ...
            if key in self.db_dict:
                ...
                return -1
    def findargmin_online(self, eval_mode, key):
        if eval_mode == '':
            if key in self.db_dict:
                runs = self.db_dict[key]
                runs = [int(x) for x in runs]
            ...
            if key in self.db_dict:
                ...
                return -1
    def find_best_epoch_offline(self, run_id):
        path = f'{prefix}/{run}/'
        ...
        argmin = -1;
        for epoch in epochs:
            history_path = f'{path}/epochs/{epoch}/history.json'
            f = open(history_path, 'r')
            err = json.loads(f.read())['average_error']
            f.close()
            if min_error > err:
                argmin = epoch
                min_error = err
