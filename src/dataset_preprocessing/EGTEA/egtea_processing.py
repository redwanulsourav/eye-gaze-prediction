...
if __name__ == '__main__':
    try:
        os.mkdir(f'/data/rsourave/datasets/EGTEA/processed')
    except:
        print('`processed` directory already exists')

    try:
        os.mkdir(f'/data/rsourave/datasets/EGTEA/processed/videos')
    except:
        print('`videos` directory already exists')
    
    try:
        os.mkdir(f'/data/rsourave/datasets/EGTEA/processed/gaze')
    except:
        print('`gaze` directory already exists')
    
    allVideos = os.listdir(f'/data/rsourave/datasets/EGTEA/raw/screened_videos')
    videoJson = {}
    for i, video in enumerate(allVideos):
        srcVideo = f'/data/rsourave/datasets/EGTEA/raw/screened_videos/{video}'
        dstVideo = f'/data/rsourave/datasets/EGTEA/processed/videos/{video}'
        shutil.copy(srcVideo, dstVideo)
        videoJson[i] = video
    with open(f'/data/rsourave/datasets/EGTEA/processed/videos/video_order.json', 'w') as f:
    ...
    for key, videoName in videoJson.items():
        ...
        gazeFilePath = f'/data/rsourave/datasets/EGTEA/raw/gaze_data/gaze_data/{videoName.split(".")[0]}.txt'
        gazeData = parse_gtea_gaze(gazeFilePath)

        frameCount = gazeData.shape[0]
        for i in range(frameCount):
            gazeJson[key][0][i] = {}
            gazeJson[key][0][i]['x'] = float(gazeData[i, 0])
            gazeJson[key][0][i]['y'] = float(gazeData[i, 1])

    with open(f'/data/rsourave/datasets/EGTEA/processed/gaze/gaze_order.json', 'w') as f:
